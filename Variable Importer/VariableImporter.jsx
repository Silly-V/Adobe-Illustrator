/***
{
	"name" : "Variable Importer 8.0",
	"note" : "This is a building script which hopefully will end up as a new version of VariableImporter.jsx"
}
***/
...
function writeScriptImages() {
  var scriptDataFolder = SETTINGS.scriptDataFolder;
...
function getTextData(dataFile){
  var df = dataFile;
  var type = df.displayName.match(/(\.txt$|\.csv$)/i)[0].toLowerCase();
...
function getPrependPathValue(varObj, cellData){
  var folderDiv = (SESSION.os == "Windows")? "\\" : "/", fileSlashStr = "";
  cellData = varObj.url + folderDiv + cellData;
	if(!cellData.match(/^file\:\/\/\//)){
		fileSlashStr = "file:///";
	}
  cellData = fileSlashStr + (cellData).replace(/\\\\/g,"/").replace(/\\/g,"/");
  return cellData;
};
...
function getXmlFileDest(userInputObj){
	if(!SETTINGS.keepXML){
		...
		var xmlDest = File(dataFile.parent + "/" + dataFile.displayName.replace(/\.\w+$/, ".xml"))
...
function filePathInput(parent, title, dialogTitle, fileSpec){
	...
	b.onClick = function(){
		var f = File.openDialog(dialogTitle, fileSpec);
		if(f != null){
...
function validate(UIElements){
	...
	if(DATA.currentVars.length == 0){
		...
		res.problem = "There appear to have been no variable names in the data file '" + decodeURI(thisDataFile.displayName) + "'";
		res.valid = false;
		return res;
	}
...
function UIWindow(){
  ...
  var imageTestGroup = w.add("group");
  SESSION.doImageTest(imageTestGroup);
  ...
  var grp4 = presetOptionsGroup(g0);
	...
	w.onShow = function(){
		imageTestGroup.parent.remove(imageTestGroup);
  ...
  g2.alignChildren = "fill";
	...
	btn_xmlFile.onClick = function(){
		...
		if(dfStr != ""){
			...
			if(dataFile.exists){
				var xmlFile = File(dataFile.parent + "/" + dataFile.displayName.replace(/\.\w{2,4}$/,"") + "-vi_data.xml");
