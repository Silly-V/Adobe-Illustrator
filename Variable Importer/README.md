# Variable Importer Pro
For over a decade, `VariableImporter.jsx` has powered workflows for thousands of companies worldwide.
Now in 2025, we’re excited to introduce **Darty-Ai** — a revolutionary new way to control the vast majority of Adobe Illustrator’s tools, all through a simple spreadsheet interface.

![https://darty.ai](https://darty.ai/wp-content/uploads/2026/01/VariableImporter_TryDarty-1.svg)

With Darty-Ai, you can effortlessly:  
* Apply colors, strokes, and fills to objects and text
* Use nearly all appearance settings available in Illustrator’s Appearance panel — including most effects
* Control object, artboard, layer, and style attributes
* Align and distribute objects with precision
* Edit text content and properties without resetting styles or formatting
* Precisely place graphics and text files
* Transform, duplicate, group, delete, and apply pathfinder operations to objects
* Export files with full control using variables
* ...and so much more.

A free version is available — try it now at [darty.ai](https://darty.ai)

---

## 👍 Good Tutorials & Blogs

VariableImporter blogs and tutorials I have found and deem officially wholesome:

* Great Video Tutorial by Christian Da: https://www.youtube.com/watch?v=aUiRGLT8byA
* Blog by Christian Da: https://www.christianda.com/blog-variable-data.html
* Multiple DataSets Tutorial by John Garrett: https://www.youtube.com/watch?v=083kc84aSIc
* Detailed tutorial by Rob Snow: https://www.youtube.com/watch?v=gqVL6uGKvTA
* Very First blog (vintage 2015) by Stephen Marsh: https://prepression.blogspot.com/2015/03/illustrator-introducing.html
* Russian-language video: https://www.youtube.com/watch?v=KtqRPyPstw4
* Spanish-language video: https://youtu.be/9zea08XqrcQ?si=HYWtafZ_6Yn4QLQ1
* VariableImporter for floor plans, Ian Cole: https://medium.com/@ian.cole/creating-event-space-plans-with-adobe-illustrator-and-the-variable-importer-4c0e7f820905
* Automating Data-Driven Graphs Using Adobe Illustrator: https://www.radishlab.com/automating-data-driven-graphs-using-adobe-illustrator/
* VariableImporter used in a Geospatial infographic project: https://element84.com/geospatial/spatial-analysis/using-arcmap-r-and-adobe-illustrator-to-create-data-driven-infographics/

---

## ⚡ Darty-Ai Tutorials

For those trying out the new powerful Illustrator plugin "Darty-Ai", these are some growing tutorial resources.

* All Docs: https://docs.darty.ai
* Self-Adjusting Logo: https://darty.ai/self-adjusting-logo-automation-with-illustrator-darty-ai-tutorial/
* Dynamic External-SVG Earrings Video Tutorial: https://youtu.be/1FwJ2uLjVfU
* Text-Replacement and Formatting: https://darty.ai/text-replacements-and-formatting/
* Placing Images: https://darty.ai/placing-images/
* Getting Started: https://darty.ai/getting-started/
* Google Sheets Introduction: https://darty.ai/collaborative-vdp-has-arrived-darty-ai/

---

## 😳 Variable Data Wall of Shame 🫢
1. https://github.com/zawahirkashif/VariableImporter -- To his credit, he wrote OK documentation. But to his ultimate shame, he deleted the credits from the top comment of the script and posts it as his own. 👎
2. https://9724054718399.gumroad.com/l/ouvjq -- "MUHAMMAD ZAIN UL ABIDIN" sells this FREE script for a minimum of $5. This is not adhering to TOS & is being reported.
